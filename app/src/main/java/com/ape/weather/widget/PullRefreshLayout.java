...
//        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.refresh_PullRefreshLayout);
        final int type = STYLE_SMARTISAN;// = a.getInteger(R.styleable.refresh_PullRefreshLayout_refreshType, STYLE_MATERIAL);
        final int colorsId = 0;// a.getResourceId(R.styleable.refresh_PullRefreshLayout_refreshColors, 0);
        final int colorId = 0;//a.getResourceId(R.styleable.refresh_PullRefreshLayout_refreshColor, 0);
//        a.recycle();

        mDecelerateInterpolator = new DecelerateInterpolator(DECELERATE_INTERPOLATION_FACTOR);
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
        int defaultDuration = 200;//getResources().getInteger(android.R.integer.config_mediumAnimTime);
