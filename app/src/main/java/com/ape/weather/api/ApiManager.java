...
import com.ape.android.util.MxxPreferenceUtil;
import com.ape.android.util.UiUtil;
import com.ape.dynamicweather.BaseDrawer;
import com.ape.dynamicweather.BaseDrawer.Type;
import com.ape.weather.WeatherApplication;
import com.ape.weather.api.entity.DailyForecast;
import com.ape.weather.api.entity.HourlyForecast;
...
//	private static AreaData AREA_DATA;
    ...
    static final String URL = "https://api.heweather.com/x3/weather?key=yourkey&cityid=";
    ...
    public static ArrayList<Area> loadSelectedArea(Context context) {
        ...
        String json = WeatherApplication.USE_SAMPLE_DATA ? GITHUB_SAMPLE_SELECTED_AREA : MxxPreferenceUtil.getPrefString(context, KEY_SELECTED_AREA, "");
...
//					UiUtil.logDebug("USE_SAMPLE_DATA", "hourlyForecast.date.fixafter->" + hourlyForecast.date);
                ...
                doHttpGet(url);//Github上的URL需要替换和风API里面的key
                // UiUtil.logDebug("FUCK", "json->" + json);
                if (!TextUtils.isEmpty(json)) {
                    Weather weather = GSON.fromJson(json, Weather.class);
                    if (acceptWeather(weather)) {
